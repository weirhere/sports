...
3. **The slate** — college football as one section per conference and the NFL as one per division (`team.conferenceId`, Power 4 → Group of 5 → Independents → FCS → "Other" for both-sides-unknown, alphabetical within a tier), then one section each for the NBA and the NHL. Which shape a league takes is `League.slateSplitsByConference`, not a branch per league. Conference buckets follow the *fetched* divisions, so FCS only appears once someone follows an FCS conference.
