...
- `seasonseries[]`: **this season's meetings only**, and not on every league — the captured NBA and NHL summaries carry it (`{seriesScore, summary, events[]}`, four Knicks-Nets games) and college football's carries none. It is the nearest thing ESPN has to a head-to-head resource and it is nowhere near enough for one, which is why the H2H tab walks `/teams/{id}/schedule` back a season at a time instead (`ScoresProviding.headToHead(for:)`). Decoded by nothing today
- `standings`: **the two competing teams' own tables, inside the game request** (read 2026-09-21, fixture-derived). `groups[]`, one per side's group — and for college football that group is the whole conference (17 ACC entries, 18 Big Ten), each entry carrying `total` and `vsconf`, which is the matchup card's entire column set. Three shape notes, all load-bearing. **A group carries no id**: it has `header` ("2026 Atlantic Coast Conference Standings"), `divisionHeader` and `shortDivisionHeader` ("ACC") and nothing machine-readable, so the conference is resolved from the competing team standing in it. **The entry is thinner than `StandingsEntryDTO`'s**: `team` is a display string with `id` beside it, and `logo` is an array (`rel: ["full","default"]` and `["full","dark"]`) rather than a `logos` one — the stats array is the same shape, so only the team half differs. **`isSameConference` is not the conference question**: it compares `conferenceHeader`, which is `"FBS"` on both sides of an ACC-vs-Big-Ten game, so it answers `true` there. Per league: the NBA's and NHL's blocks ship the **division** (Atlantic, Pacific) with 4–5 stats and no `total` summary (NBA) or `gamesplayed` (NHL). No NFL summary has been captured.
- `header.competitions[0].competitors[].team.groups`: `{id}` and no `conferenceId` — the scoreboard's team object is the other way round. Bare here (`{"id": "1"}`, no parent); the same field in `/rankings` names the division root as parent (`{"id": "5", "parent": {"id": "80"}}`) and neither marks `isConference`, which only the schedule endpoint's copy does.
...
### Athletes and stats — a different host (verified live 2026-09-24)

**Use `site.web.api.espn.com` for anything about an athlete or a season's stats.** On `site.api.espn.com` — the host every other request here uses — `/athletes/{id}`, `/teams/{id}/statistics` and even `/teams/{id}/roster` sit behind an Akamai rule that answers **403 "Access Denied" depending on the User-Agent**: a browser UA or an empty one is refused, a CFNetwork UA mostly gets through. `/athletes/{id}` is a 404 on that host regardless. When probing with `curl`, send a CFNetwork-style UA or every one of these reads as blocked.

- **`/apis/common/v3/sports/{sport}/{league}/athletes/{id}`** — the profile (`AthleteProfileClient`): age, jersey, position, height, weight, headshot, status, `team.id`.
- **`…/athletes/{id}/stats`** — `categories[]`, each with `name` (`passing`, `averages`, `goaltender`, …), `displayName`, `labels[]`, `names[]` (stable keys — match on these, not labels), `displayNames[]` (full names — "Passing Yards"; not `descriptions[]`, which are glossary sentences), `statistics[]` (one row per season: `season.year`, `season.displayName`, `teamId`, `position`, `stats[]`) and `totals[]` (ESPN's career line). ESPN orders the categories by what the player does, so `categories[0]` is theirs. Top-level `teams` maps slugs to club names. Some athletes ship no `categories` at all. An NFL player's `filters` offer `league=college-football` too — their college career is one request away.
- **`…/athletes/{id}/gamelog?season={espnYear}`** — `labels[]`/`names[]` for one flat row per game (football repeats "YDS" and "TD"; top-level `categories[]` gives the column groups with counts), `events{eventId: {week, atVs, gameDate, homeTeamScore, awayTeamScore, gameResult, opponent, team, eventNote}}`, and `seasonTypes[]` (Regular Season / Postseason) whose `categories[]` hold the `events[{eventId, stats[]}]` — by month in the NBA/NHL, one bucket in football. `gameDate` has fractional seconds and an offset. `filters[name=season].options` lists every season ESPN will answer for. `season` is ESPN's year: the ending year for the NBA/NHL.
