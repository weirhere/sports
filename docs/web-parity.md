...
| 2026-09-13 | Game detail gains an H2H tab — a windowed series tally over the previous meetings | pending — the web has the same `HeroTabBar` and the same `GameRow`, so the pane and the `HeadToHead` reducer port almost line for line (`web/src/lib/` has no equivalent yet; the tally rules are the part worth copying, not re-deriving). What does **not** port is the fetch's shape: `/game/[league]/[gameId]` is a server component, so an eager series would spend ~20 upstream requests on every page render including the ones nobody opens the tab on — where iOS spends them on an explicit tap. The web needs the walk behind a route segment or a client fetch first, which is the work this row is waiting on. `teamScheduleUrl` in `web/src/lib/espn/endpoints.ts` already has the season axis it needs |
| 2026-09-13 | A team page gets a **Trophies** tab — trophies derived from `notes[].headline` on the games a team played, one card of count-and-years rows, with `TrophyRegistry` shipping empty behind a "since 2014" caption | **pending** — the iOS half hasn't been through a compiler yet (no Xcode in that session) and the rules want an eyes-on pass on a real slate first. The web has the **identical decode gap** and it is one line: `transformScheduleEvent` never sets `headline`, though `Game` carries the field and the scoreboard transformer at `transformers.ts:381` sets it — so a web team page cannot name its own title games either. The rest is a real port rather than a line: this page is server-rendered per season and has no multi-season fetch, where iOS fills its schedule cache back to the floor when the tab opens, so the web needs either a client fetch on tab open or a server fan-out. Rules to mirror verbatim: a headline is not a trophy, a series is decided by its last completed game, a qualified round ("Quarterfinals", "Semifinals") decides nothing, and a title game is named after the trophy rather than the fixture |
