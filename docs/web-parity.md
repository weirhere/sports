...
| 2026-09-18 | The hero tab row scrolls horizontally rather than wrapping its labels | **shipped** — `hero-tab-bar.tsx` gets `overflow-x-auto px-4 scrollbar-none` on the tablist and `shrink-0 whitespace-nowrap` on each tab, and the two wrappers that used to supply the gutter (`hero-header.tsx`'s tab slot, `game-detail-view.tsx`'s tab strip) drop their `px-4`. Same defect on the same component: at 390px a five-tab team page measured 461px of content, so the flex row wrapped the labels. Verified at 390px in the browser — one 49px line, every tab on one line, the row scrolling to Trophies. Roving tabindex is unchanged and `focus()` scrolls an off-screen tab into view for free. No analog for the iOS `ScrollViewReader` re-centre: the web has no content swipe that changes tab from off-screen |
