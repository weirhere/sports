...
| 2026-09-21 | Root tabs share one masthead, with a 52pt row floor | **pending** — `web/src/components/nav-bar.tsx` and the page headers are per-page there too. The 52pt floor is an iOS tap-target number; the web's equivalent is whatever keeps its own control row on one line, so the *rule* ports and the value does not. |
...
| 2026-09-21 | Find a league filters the hub and expands what it matched | **pending** — straightforward there, and the "expand what matched" half is the part not to drop: the web hub's accordions would need the same render-time override rather than a write to stored state. |
