...
| 2026-09-20 | Roster rows link to a player page; the page is Profile only, with no tab row until a second tab can be filled | **pending** — the web has the same three surfaces and the same athlete id space, and its roster rows are non-links for the identical reason. The port is a route, a page and the same `PlayerIdentity` shape; it inherits the gate too, so it should wait for the probe rather than shipping a Profile-only page on a different day |
