...
| 2026-09-20 | A box score name links to the player page, on a genuine athlete id only | **pending** — web only, and correcting an overstatement in this row's first version: iOS is *not* model-ready. `BoxScore.Player` has no `athleteID` field at all, so the two-field split exists on the web alone (only the empty-string fix landed on both). The port is also not just a build away, which is the more useful half: the web link re-fetches the team's roster from the URL, so the page arrives whole, while iOS carries facts in memory and a box score row knows a name, a number and a stat line — `PlayerIdentity`'s own doc comment says a page built from that "would be the row again". So iOS's box score door wants the athlete fetch (E20 P0) before it opens, not a `NavigationLink` |
...
| 2026-09-21 | The game page reads its Standings card out of `/summary` instead of fetching the tables | **pending** — the web's game page fetches standings the same way (`web/src/app/(main)/game/[gameId]`), and the saving is larger there: a server-rendered page pays the round trip before it can respond. Same per-league cut applies, and the same fallback. |
...
| 2026-09-21 | A team decoded from a summary header or a ranking learns its conference from `groups` | **pending** — the web's `transformTeam` has the same gap against the same two payloads. Worth porting with the row above, since the standings change depends on it. |
| 2026-09-21 | The hero tab row's inactive ink moves from 50% primary to `textSecondary` | **pending** — the web's `HeroTabBar` separates its tabs the same way and its `text-primary/50` composites to the same place on `bg-bg-card`. Same fix, same token: the web owns a `text-text-secondary` with the same job. Worth measuring rather than assuming, since the web's ramp is defined in OKLCH and its numbers won't be these numbers. |
