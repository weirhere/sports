...
| 2026-09-12 | The Today button scrolls the slate home as well as moving the strip, and re-bounds a past-season strip | pending — the web has the same floating button (`today-button.tsx`) and the same hole, worse: a client route/state change doesn't reset window scroll at all, so today arrives exactly where the previous day was left. The past-season half is the one piece with no web shape yet — `web/`'s Games route has no season selector to strand the strip in the first place |
| 2026-09-12 | The Live filter applies to today and nowhere else — off today it is suspended and the pill turns off with the day; coming home turns it back on | pending — the web has both halves already: the Live pill lives in the Games controls card (2026-09-10) and the slate has the day swipe the card deliberately sits outside of. The port is the same shape, one layer up from storage: keep the stored toggle as the intent, derive the effective filter per day, and read it per day so the neighbouring day renders unnarrowed. The web's filter state is in the URL/store rather than `UserDefaults`, so the derivation is where this lands, not the persistence |
...
| 2026-09-20 | Tapping the Games tab while already on Games pops to the slate and scrolls it to the top | **pending** — the web has the same gap with a different shape. `BottomTabBar`'s Games entry is a `next/link` to `/`, and clicking it from `/` is a same-route navigation the router treats as a no-op, so a scrolled slate stays scrolled; the desktop `NavBar` is the same. The iOS half of the decision (popping a pushed page) has no web analog — a game detail is its own route, so the link already leaves it — which leaves one change to port: an `onClick` that scrolls the window to the top when the tab's href is the current pathname |
...
| 2026-09-21 | A Games tab re-tap returns the slate to today, not just to the top | **pending** — a real analog, not done. `web/src/components/bottom-tab-bar.tsx` carries the same Games tab at `/`, and a Next.js `Link` to the route you are already on does nothing, so the web has neither half of this: not the 2026-09-20 scroll-to-top and not today's day reset. Whoever ports it inherits one wrinkle iOS does not have — the web tab is a link rather than a selection binding, so the re-tap has to be caught in an onClick that compares `pathname` before the router short-circuits it. |
| 2026-09-21 | The search page is rebuilt: left title, scope pills, one card per result, field at the bottom | **pending** — real and not done. `web/src/app/(main)/search` keeps the old shape. The bottom-anchored field is the half that does not port directly: a browser has no keyboard inset to ride, so the web equivalent is a sticky footer bar, which is a different mechanism for the same reason. |
| 2026-09-21 | Search opens on recents; teams and games resolve live, players snapshot | **pending** — the web has no recents at all. Its storage question is different too: `localStorage` per browser rather than `UserDefaults` per install, so the "survives a reinstall" answer differs and should be decided rather than inherited. |
| 2026-09-21 | Athletes in search, via ESPN's search endpoint, filtered to FBS and FCS | **pending** — and the probe that unblocked this unblocks the web identically, since the endpoint is league-agnostic and needs no key. The directory filter ports as-is: `web/` loads the same standings-derived directory. |
...
| 2026-09-21 | CFB becomes NCAAF in display; the spoken form stays "College Football" | **pending** — `web/src/lib/leagues.ts` still says CFB and is now out of step on every screen. The spoken half matters there too: a screen reader on the web reads the same initialism, so the `aria-label` wants the phrase where the visible text wants the abbreviation. |
...
| 2026-09-21 | Games search reaches past the loaded window via the matched teams' schedules | **pending** — and the web starts from the same place: its games corpus is the loaded slate too. The endpoint and both rules port unchanged, since `/teams/{id}/schedule` needs no key and the merge-and-order logic is platform-free. The one thing not to port is the three-team cap as a number: it is a phone-sized answer to "how many requests is a keystroke worth", and a browser with a warm cache may reasonably choose differently. |
...
| 2026-09-22 | Hide all/Show all control on Scores, scoped to sections you don't follow | **pending** — `web/src/lib/game-sections.ts` already builds the same followed-first ordering (`followed-tables.ts`), so the web's port is a boolean flag on the sections it already produces plus a control between the two groups; no new grouping logic needed there either. |
...
| 2026-09-24 | Section open/close is a 0.18s ease-out, not a spring | **pending** — check the web accordion's transition; if it springs or runs longer than ~200ms, match the curve. |
