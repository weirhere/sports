...
| 2026-09-21 | The Leagues hub drops its standings teasers and puts its rows on one rail | **pending** — `web/src/app/(main)/rankings` carries the same teasers and the same three row types. The rail half ports as a rule rather than a number: the web has no 26pt box, but it does have three row types that must share one mark size and one gap, and the iOS bug — fixing two of three and creating a third misalignment — is the one to avoid repeating there. |
...
| 2026-09-25 | Leagues Following: Edit/Done link on the heading; dismiss + grip only in edit mode; no star or hold-to-lift outside it; drop resolved against visible neighbours | **pending**: the web keeps its hold-to-drag until `web/src/components/followed-tables-list.tsx` gets the same mode. The visible-neighbour index fix applies there too. |
| 2026-09-25 | Leagues Following cards join the league onto the name ("SEC - NCAAF"); bare when the title is the league's own name | **pending**: ports with the Edit/Done row above, reusing the web section header's join from #198. |
