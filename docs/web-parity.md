...
| 2026-09-21 | The Leagues hub drops its standings teasers and puts its rows on one rail | **pending** (partly shipped 2026-09-25: the web port of Following's Edit/Done dropped the teasers from the Following cards) — `web/src/app/(main)/rankings` carries the same teasers and the same three row types. The rail half ports as a rule rather than a number: the web has no 26pt box, but it does have three row types that must share one mark size and one gap, and the iOS bug — fixing two of three and creating a third misalignment — is the one to avoid repeating there. |
