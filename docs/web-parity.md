...
| 2026-09-20 | A box score name links to the player page, on a genuine athlete id only | **differs** — the split is on both platforms, the link is on one. `BoxScorePlayer.athleteId` and iOS's model-side equivalent both distinguish the real id from the synthesized key, and the empty-string fix landed in both; only the web draws the link today, because `BoxScoreList`'s `NavigationLink` could not be compiled or run from the session that made the change. iOS row is model-ready, `pending` on a build |
