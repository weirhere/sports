...
| 2026-09-17 | The loading skeleton renders on cards; a day whose fetch failed shows the error and a Retry instead of an endless skeleton | **shipped** — this row runs backwards, like the 2026-09-10 wordmark. `game-card-skeleton.tsx` already wraps each section in `card-surface`, so the web never had the light-mode `--bg-elevated`/`--bg-recessed` collision (both are `#ededed` there too), and `(main)/error.tsx` is a real error boundary where a thrown fetch lands — a server-rendered page cannot sit under its suspense fallback forever the way the client-side store could. iOS is the one catching up |
