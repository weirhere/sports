...
| 2026-09-21 | The Leagues hub drops its standings teasers and puts its rows on one rail | **pending** — teasers **shipped** 2026-09-25: the Following cards dropped theirs in the Edit/Done port (#203), and the league accordion rows dropped the conference leader and the Top 25 row's "#1" (and its spoken label) after. Still open from the same iOS row: the table count on the league headers, the header's type step, the heading below Following reading "All leagues, conferences, divisions", and Top 25 wearing a trophy. The rail half ports as a rule rather than a number: the three row types must share one mark size and one gap — they line up on the web today, so keep them that way. |
