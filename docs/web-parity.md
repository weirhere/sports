...
| 2026-09-26 | Live polls run every 5s, not 30s | **pending** — web's `POLL_INTERVAL_LIVE` (client, `use-live-game.ts`) and the provider's 30s `next.revalidate` on the scoreboard and summary fetches both need to drop together; the client interval alone would just re-read a 30s server cache. |
