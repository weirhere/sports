...
| 2026-09-10 | The team page gains a Roster tab (FotMob's squad shape, per-league metric column, current-season only) | **pending** — the web has a team page with Overview/Games/Standings and no roster. The three ESPN findings port unchanged: the NBA's flat `athletes` array, the missing season axis, and the CLASS-vs-AGE column. So does `URL.headshotThumbnail` — the web would pull the same ~200 KB press photos without it |
