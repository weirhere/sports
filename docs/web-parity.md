...
| 2026-09-21 | Teams read by full name with "LEAGUE • Conference" beneath | **pending** — the web's team rows and hero carry the short location. Worth porting as one change with the NCAAF row above, since the subtitle contains the league name. |
...
| 2026-09-21 | A team in the Add teams sheet reads as one name | **pending** — wherever the web's follow list sets a location against a nickname. The wrap-don't-truncate half matters more there, not less: a browser column can be narrower than a phone's. |
