...
| iOS date | Decision | Status |
|---|---|---|
| 2026-09-10 | A season is fetched in as many windows as it takes; every conference page gets a Games tab | **shipped 2026-09-25** — `canTableAWholeSeason`, `GAMES_WINDOW` and `rollingConferenceGames` are gone and every league's conference page tables its whole season month by month (10 requests for the NBA/NHL), over a `seasonGamesSpan` that also covers the 2019-20 and 2020-21 pandemic overruns iOS's span still clips |
...
| 2026-09-12 | The Today button scrolls the slate home as well as moving the strip, and re-bounds a past-season strip | **shipped** — `scores-view.tsx`'s `jumpToToday` routes the button through `selectToday` (which re-bounds a past season picked in the filter sheet) and scrolls the window to the top in the same frame |
| 2026-09-12 | The Live filter applies to today and nowhere else — off today it is suspended and the pill turns off with the day; coming home turns it back on | **shipped** — the stored toggle stays the intent and `narrowsOn(intent, day)` in `use-ui-state.ts` is the effective filter; the pill, the slate and the empty state read it per day, the "Ongoing" rename reads the intent, and a click while suspended turns it on and goes to today |
| 2026-09-12 | The calendar sheet opens on the selected day's month, not the season's first | **shipped** — `day-calendar-sheet.tsx` renders every month eagerly and scrolls the selected cell into view on open, so the web has never had this bug. What iOS fixed is a SwiftUI lazy-stack and scroll-timing failure with no DOM analog; the behavior both sides promise is the same |
...
| 2026-09-10 | The wordmark reverts to plain 17-bold system type (no condensed axis, no weight split, no tracking); the 🏈 stays gone | **shipped** — iOS settled it at 21pt heavy on the masthead, and `web/src/components/wordmark.tsx` now matches: 21px, weight 800, no `tracking-tight`, via the `type-page-title` token it shares with `PageHeader`. |
...
| 2026-09-12 | The active Live chip wears the accent instead of the ink — a `liveTint` wash under a `liveEdge` hairline, `textPrimary` staying put, dot up to 8pt | **shipped 2026-09-12** — same commit, both platforms. `web/src/components/scores-header.tsx` already carried the iOS chip line for line (ink fill, inverted label, 6px dot), so it took the same two tokens: `--live-tint` / `--live-edge` in `globals.css`, mirroring `Color.liveTint` / `Color.liveEdge`. The funnel beside it keeps the ink fill and gains a transparent border so the two pills stay the same height. Reads as a pair with the selected-day row above, which dropped its fill rather than recolouring it |
| 2026-09-13 | The Game info card stays on the Summary tab once a game starts | **shipped 2026-09-13** — same commit, both platforms. `game-detail-view.tsx` drops the `!scores &&` half of its gate, so the rail renders `GameInfoCard` at every state. One difference, and it is the web's layout rather than a decision: on desktop the pair lives in the right-hand context rail, where `VenueCard` was already ungated — so the web's Venue card does not do iOS's pre-kick-up-top / played-down-below move, and the two now simply sit together. Pre-existing, unchanged here |
| 2026-09-13 | A team's Overview tab gets a Venue card — home ground, city, home dates and average attendance, derived from the schedule it already fetched | **shipped 2026-09-13** — same commit, both platforms. `deriveHomeVenue` in `transformers.ts` mirrors `TeamVenue.home(from:)` rule for rule (modal ground, neutral sites out, ties to the earlier date, gates of zero excluded), `TeamScheduleData` gains the same optional `homeVenue`, and `team-venue-card.tsx` is `TeamVenueCard`. `venue-headline.tsx` is the same extraction, so the game page's `VenueCard` now shares its headline too. One thing the web did *not* need: it already transformed a schedule competition's venue into its `Game`; only `attendance` had to join `EspnScheduleCompetition` |
| 2026-09-13 | Game detail gains an H2H tab — a windowed series tally over the previous meetings | **shipped 2026-09-13** — `head-to-head.ts` is the `HeadToHead` reducer line for line (the anchor excluded and nothing after it, final-only, the winner flag behind the scores, ties counted rather than dropped, `windowLabel` travelling with the numbers), `head-to-head-pane.tsx` is the pane, and `GameRow` takes the same `showsYear`. The tab is offered from the first frame on both sides, so a pre-kick page now carries a two-tab row here too. **The fetch is where it differs, and the row above called it:** a server component can't spend 20 upstream requests on every render, so the walk sits behind `GET /api/game/[gameId]/head-to-head` and runs on the tab's first open — which is what iOS's explicit tap already bought it. The route re-reads the anchor from the summary rather than trusting a client's team ids, and pays nothing for it (Next hands back the page's own cached response). `teamSeasonGames` skips the preseason *request* rather than filtering it after, so college football's ten seasons cost 20 requests, not 30; `walkSeasons` is the three-wide pool, dropping a failed season and throwing only when every one failed — the distinction that lets "no meetings" mean it |
| 2026-09-13 | A team page gets a **Trophies** tab — trophies derived from `notes[].headline` on the games a team played, one card of count-and-years rows, with `TrophyRegistry` shipping empty behind a "since 2014" caption | **shipped 2026-09-13** — the decode gap was the one line the row predicted: `EspnScheduleCompetition` now declares `notes` and `transformScheduleEvent` sets `headline` from it, which `georgia-schedule-2025.json` (a trimmed capture of the real 2025 payload, SEC Championship and all) holds a regression test on. `trophies.ts` mirrors every rule verbatim — a headline is not a trophy, the league title is checked before the conference round, a qualified round decides nothing, a series is decided by its last completed game, a title game is named after the trophy — and `trophy-registry.ts` ships empty behind the same gate, so a web page reports what the wire says and captions it. The tab is unconditional here too, for its own reason: the answer costs a dozen season requests. **Fetch, as predicted, is the real work:** `GET /api/team/[teamId]/trophies` walks the seasons on the tab's first open, where the page itself stays server-rendered per season. Two things the web didn't need: the schedule cache (each season's pair sits in Next's fetch cache for an hour, shared across visitors) and a season chip to suppress — the tab simply isn't in the chip's list. **Neither half has been seen against live ESPN**: the egress policy blocks it from this session exactly as it did the one that built the iOS side, so both want the same eyes-on pass on a real slate |
| 2026-09-13 | The NFL's standings table becomes ESPN's own twelve-column spread, with the identity half pinned and the numbers scrolling | **shipped 2026-09-13** — same commit, both platforms. `standings-columns.ts` takes the same NFL set and the same `standingsScrollsHorizontally` gate, the transformer reads the same nine stats off the payload it already fetched, and `standings-list.tsx` pins with CSS instead of a second column: the identity span goes `sticky left-0` with a `bg-bg-card` ground inside an `overflow-x-auto` wrapper, which is the web getting for free the row-height agreement iOS has to state. Two things the web did **not** need — the tab-swipe stand-down (its entity pages have no swipe; tabs are links), and a `matchupStandingsColumns` helper, because the game page's matchup card has hardcoded its own CONF/OVR pair since it was written. One difference worth naming: the pinned column is 168px on web at every width, where iOS scales it with Dynamic Type |
| 2026-09-13 | The privacy policy and the support page become routes on statside.co | **shipped 2026-09-13** — web-originated, so this row runs the way the CTA and the OpenGraph card did: there is no iOS side to catch up to, and the iOS half of the change is that `PRIVACY.md` (which the app has no screen for) is now the source a published page mirrors. `/privacy` and `/support` are static routes in `(main)`, so they inherit the nav bar, the tab bar and the Get-the-app pill; `prose.tsx` + `lib/markdown.ts` are the renderer, `site-footer.tsx` is new chrome on **every** route, and the bottom padding that clears the fixed tab bar moved from `<main>` to the footer — with it left on the content, every page showed 80px of dead space above the new hairline |
| 2026-09-13 | statside.co keeps Vercel Web Analytics and the privacy policy discloses it | **shipped 2026-09-13** — and `n/a` in the direction that matters: there is nothing to port, because the iPhone app has no analytics and the decision is precisely that it still doesn't. The policy now answers under two headings rather than one, so "**Nothing.**" stays true of the app while the website's page-view counting is stated where a reader can check it |
...
| 2026-09-10 | The iOS wordmark reverts to plain system type (21pt header, 15pt share sign-off), no glyph | **shipped** — this row runs backwards: the web reverted first, on 2026-09-10, and iOS is the one catching up. `web/src/components/wordmark.tsx` is the reference implementation. The sizes matched on 2026-09-25: the web nav bar went from 17 to iOS's 21, weight 800, with the root-tab masthead port. |
| 2026-09-12 | The Top 25 header's name opens the poll — the Scores accordion split now covers every section naming a real table, not just conferences | **shipped** — both halves landed in the same PR. The web had the identical dead header (`section-accordion.tsx` linked only `section.conference`) and `/rankings/poll` was already a route, so the fix was the same shape: one `nameHref`, one aria-label that says what the tap does |
| 2026-09-15 | The rating ask, armed by a kickoff-reminder tap and spent when that game's detail page loads | **n/a** — `requestReview` is StoreKit asking about an App Store listing, and the web app has no listing, no notification to arm it and no store to rate. The nearest web analog would be a "get the app" nudge, which `GetTheAppPill` and `GetTheAppCard` already are (2026-09-10). Nothing to port |
| 2026-09-15 | `aps-environment` restored to the app entitlements, ahead of the broadcast service | **n/a** — an iOS code-signing entitlement. The web half of Live Activities is the sending service in `web/` (already `shipped`, 2026-09-10); the receiving end is the app, and a browser has no entitlements file, no APNs topic and no lock screen to put a card on |
| 2026-09-15 | The broadcast tick is 60s on an external pinger; `.env.example` documents the service's six variables | **shipped** — it *is* web code, the second row to run in that direction after the service itself (2026-09-10). What landed here is a comment correction and env documentation in `web/`; the iOS half has no scheduler to reach parity on, since the whole point of path 3 is that the phone is a subscriber rather than a poller |
| 2026-09-15 | The APNs transport moves to `node:http2`; send failures report a reason instead of throwing; the poll route accounts per league | **shipped** — it *is* web code, the third row running server-half-first after the service (2026-09-10) and the pinger cadence. The iOS client has no analog: it subscribes to a channel and never sends, which is the whole point of path 3 |
| 2026-09-15 | `apns-expiration` required on every broadcast, set from the game's clock (update → stale date, final → an hour) | **n/a** — an APNs wire requirement on the sending half. The web app sends no pushes; the header has no browser analog, and the receiving end is the iOS card |
| 2026-09-15 | APNs accepted a broadcast; the service's "never run against Apple" status retires | **shipped** — the milestone belongs to `web/`, which is where the service lives. Nothing to port: the web app is the sender, and the receiver is an iOS lock screen |
| 2026-09-17 | The loading skeleton renders on cards; a day whose fetch failed shows the error and a Retry instead of an endless skeleton | **differs** — the two halves split. *Rendering*: the web never had it, and this row runs backwards like the 2026-09-10 wordmark — `game-card-skeleton.tsx` already wraps each section in `card-surface`, so its bars sit on white despite `--bg-elevated` and `--bg-recessed` both being `#ededed`. *The stranded error*: **the web had it too**, and worse. `(main)/error.tsx` catches nothing here, because `page.tsx` catches per league and returns `undefined` rather than throwing. `scores-view.tsx` gated the error branch behind `!isLoaded` exactly as iOS did, and `use-league-scoreboards` only set an error when **every** league failed, so a partial failure held the skeleton up forever with the app believing nothing was wrong. Corrected the same day, from Andy's second screenshot; see the row below for the fix |
| 2026-09-17 | The web half: a stalled day shows ESPN's status and a Retry, a partial failure is an error, and the server seed logs why | **shipped** — `scores-view.tsx` gets the iOS `isStalled` gate, `use-league-scoreboards` reports any failed league rather than only a total wipe-out, `/api/scoreboard` forwards ESPN's status as `upstreamStatus` (the 502 stays a 502), `lib/api.ts` raises a typed `ApiError` carrying it, and `page.tsx`'s silent per-league `catch {}` now logs. The iOS half has no analog for the last two: it talks to ESPN directly, so `ESPNError.badStatus` was always in hand and there is no route to forward through and no server log to write to |
| 2026-09-17 | ESPN withdrew the `dates=A-B` range form and dropped `limit`'s ceiling to 500; the client fans out per day for a window and per month for a season | **shipped** — both halves, same day, see the row below. The web had both breakages and one extra: `espnDayRange()` built the dead hyphenated form for two callers, both window builders defaulted to `limit=900` above the ceiling, and `conferenceGames` still split its season at November 1, the college-football date iOS had already retired for the halving rule on 2026-09-10 |
| 2026-09-17 | The web half: `espnWindow()` replaces the range form, both window builders drop to `limit=500`, and a conference season is fetched month by month | **shipped** — `scoreboardForDays` and `conferenceGames` fan out and merge by event id, clipped back to the span by `clipGames`. The November 1 split is gone. 479 tests pass, typecheck, lint and `next build` clean, and all four leagues verified 200 through `/api/scoreboard` |
| 2026-09-19 | A screenshot never names the price, including "free" | **n/a** — App Store listing metadata. The web has no screenshot set and no store review to pass; `statside.co` is free to say the app is free, and `grep -i free web/src` finds the word only in three code comments. The iOS Description keeps "Free." too, so there is nothing here even to mirror |
| 2026-09-19 | A roster group is ordered by jersey number, the numberless behind it | **shipped** — same commit, same rule: `sortedByJersey` in `transformers.ts` sorts both the grouped and the flat payload's players before they become groups. `Array.sort` is stable by spec, so the tail keeps ESPN's alphabet without the hand-written index tiebreak iOS needs. Three tests in `roster-transformers.test.ts`; 482 web tests, typecheck and lint clean |
| 2026-09-20 | The entity-page tab strip scrolls itself instead of widening the page, and `<main>` clips what still overflows | **differs** — the web had the bug and iOS cannot have it. This row runs backwards: a too-wide flex row widens the document, and there is no SwiftUI equivalent of a document that grows past the screen, so the iOS `HeroTabBar` has never shifted anything. The shared half is the underlying shape — both are a bare row of five tabs with no scroller — and iOS pays for it differently, in compressed or truncated labels. Filed in BACKLOG E5, unverified, rather than ported: it needs a simulator pass at `large` and at an accessibility size before a pinned strip gains a `ScrollView` |
| 2026-09-20 | The widget footer is a manual-refresh button, the live ask drops to 5 minutes and the quiet tick rises to 4 hours | **n/a** — home-screen widget, start to finish. The web has no widget, no WidgetKit budget and no timeline to schedule; `statside.co` refreshes because a browser tab is open and the client polls, which is the thing a widget cannot do and the reason this decision exists. Nothing to mirror, and the 5-minute number carries no meaning off iOS — the web's live poll is its own decision with its own arithmetic |
| 2026-09-20 | Tapping the Games tab while already on Games pops to the slate and scrolls it to the top | **shipped** — the tab bar's and the desktop nav's Games links catch the re-tap in `onClick` (`scores-home.ts`, pathname compared before the router no-ops it) and send a window event the Scores view answers by scrolling to the top; modified clicks are left to the browser |
| 2026-09-20 | Roster rows link to a player page; the page is Profile only, with no tab row until a second tab can be filled | **shipped** — same day, same shape: `src/lib/player-profile.ts` mirrors `PlayerIdentity`'s helpers assertion for assertion (13 tests against iOS's 12, the extra ones covering the route), `RosterList` takes a `teamId` and its rows become `next/link`s with a chevron, and `HeroHeader` handed no `tabs` renders the no-tab-row page without a new component. The web hero's logo slot is 56px where iOS's headshot is 76pt — the web hero's own measurement, not a new decision |
| 2026-09-20 | A player URL carries the team id: `/player/{league}/{teamId}/{athleteId}` | **n/a** — this is the web's decision and iOS has no analog. A SwiftUI destination carries the player's facts in memory, so nothing has to be rebuilt from an identifier; a URL has to survive being shared, refreshed and crawled, with no athlete endpoint to rebuild from |
| 2026-09-20 | A hero rendered with no tab row carries its own bottom padding | **differs** — and deliberately, because the bug is the web's alone. iOS `PlayerPage` builds its hero inline on `bgRecessed` with `Spacing.lg` to the Profile card; the web's `HeroHeader` is a `bg-bg-card` surface whose bottom edge was drawn by whatever followed it, which is the tab row everywhere but the player page. Fixed on the web component, no iOS analog to port |
| 2026-09-20 | A box score name links to the player page, on a genuine athlete id only | **shipped** — closed from the iOS side: the web linked first (`BoxScorePlayer.athleteId`), and iOS gained the athlete id and the link on 2026-09-24 (see that row below). Nothing left on either platform. |
| 2026-09-20 | An empty athlete id falls back like a missing one (`nonEmpty`, not `??`) | **shipped** — both platforms, same commit. `transformers.ts` and `ESPNClient.swift:1592` had the identical line and the identical bug |
| 2026-09-20 | The team on a player's hero is a badge linking to the team page | **differs** — web only, and the web is where the hero has a badge to draw. iOS `PlayerPage`'s meta row is a `LogoImage` and a `Text` inside an `accessibilityElement(children: .ignore)`, so the same change there is a different shape (a `NavigationLink` and a rebuilt accessibility tree, not a pill) and wants a build to verify. `pending` for iOS |
| 2026-09-20 | iOS: the player hero's team becomes a badge pushing `TeamPage` | **shipped** — the web shipped its half the same day and this is iOS catching up, so the row closes on arrival. One deliberate divergence in form: the web pill carries the crest inside it, iOS keeps the crest beside a text-only `HeaderLinkBadge` because that is the component both its other headers already use. Same idea, each platform's own vocabulary. The web's accessibility fix (badge outside `aria-hidden`) and iOS's (sentence off the container, onto the title) are the same fix in two dialects |
| 2026-09-20 | `HeaderLinkBadge`'s capsule fill becomes a parameter | **n/a** — a Theme-layer component the web has no twin of. The web's badge is a Tailwind pill on the hero's `bg-bg-card` ground, where `bg-bg-elevated` already separates, so the collision this fixes cannot arise there. Recorded rather than ported |
| 2026-09-20 | The Leaders card carries the athlete id, and the web links on it | **differs**, on purpose. Both platforms carry the id; only the web routes on it. A URL rebuilds the page by re-fetching the team's roster, so a web Leaders tap arrives at a whole page; iOS carries facts in memory and a leader row holds a name, a stat line and a photo, which would render a hero over an empty Profile. Same split, same reason, as the box score door. iOS links when E20's P0 lands |
| 2026-09-20 | `transformLeaders` exported for testing | **n/a** — a web-only module-visibility change. iOS's `ESPNMapper.leaders(from:)` is already reachable from `sportsTests` through `@testable import` |
| 2026-09-21 | The game page reads its Standings card out of `/summary` instead of fetching the tables | **shipped** 2026-09-25 — `transformMatchupStandings` reads the block into `GameDetail.matchupStandings`; the page skips the fetch for college football only (`summaryCarriesMatchupStandings`) and fires it once, sequentially, when the block is missing. |
| 2026-09-21 | `isSameConference` stays unread; same-conference is derived from the two teams' tables | **n/a** — the flag is in a payload the web decodes too, and the reason not to trust it is the payload's, not the platform's. Recorded so the web doesn't independently discover it and reach the opposite conclusion. |
| 2026-09-21 | A team decoded from a summary header or a ranking learns its conference from `groups` | **shipped** 2026-09-25 — `conferenceIdFromGroups` takes the league and reads a bare group, or one whose parent is a division root, as the conference itself. |
| 2026-09-21 | The hero tab row's inactive ink moves from 50% primary to `textSecondary` | **shipped** 2026-09-25 — inactive tabs take `text-text-secondary` (hover to primary); measured on `bg-card`, the old ink was 3.34:1 light / 5.17:1 dark and the token is 5.33:1 / 6.36:1 (the tokens are hex, so iOS's numbers carried over). |
| 2026-09-21 | The game share is one rich bubble via `LPLinkMetadata` (recorded late) | **n/a** — a sender-supplied preview is an iOS share-sheet mechanism with no browser analog, and the web's own answer shipped separately on 2026-09-09 as `opengraph-image.tsx`, which is server-side unfurling and already has its row. Recorded here only so the ledger's count matches the log's. |
| 2026-09-21 | The Live Activity client surface, recorded late | **n/a** — ActivityKit is an iOS lock-screen and Dynamic Island surface with no browser analog. The web's nearest equivalent question is push notifications, which nothing in this project has scoped. Recorded so the ledger's count matches the log's. |
| 2026-09-21 | The backlog is reconciled against the code on a cadence; PR template gains a backlog-closure section | **n/a** — `BACKLOG.md` and `.github/pull_request_template.md` are one repo's process, shared by both platforms already. Two of the six corrections were *about* web parity rows (E8's `Division` clause and half of E15's), and both closed as already-shipped on the web, which is the ledger working. |
| 2026-09-21 | A trophy's identity folds case; ESPN's shouted "NFC CHAMPIONSHIP" is not a second trophy | **shipped** — ported in the same commit. `web/src/lib/trophies.ts` had the identical bug from the same source: it keyed both `deriveTrophies` and `assembleTrophyCase` on `kind.singular` verbatim, so the Rams' shelf would have split the same way. It now keys on a `${tier}\0${singular.toLowerCase()}` composite, re-cases the same one word, and picks a merged row's spelling by the same shoutiness rule; `registryCoveredKinds` folds to match. Both suites cover the Rams' pair directly. |
| 2026-09-21 | The iOS hero tab row scrolls itself; labels wrap rather than truncate without it | **shipped** — the web took this fix first, on 2026-09-20, and this is iOS catching up to it rather than the usual direction. No web work falls out: `HeroTabBar.tsx` is already an `overflow-x-auto` scroller with `shrink-0` buttons and its own `px-4` gutter, and `<main>` already carries the `overflow-x: clip` guard. One divergence worth knowing if the two are ever reconciled: web reveals a clipped tab nearest-edge with a 16px gutter, iOS centres it on `DayStrip`'s rule. Both are correct for their platform; neither is a parity gap. |
| 2026-09-21 | A Games tab re-tap returns the slate to today, not just to the top | **shipped** — the same event goes through `jumpToToday` whenever the hook's new `canJumpToToday` says there is somewhere to go, so the offseason re-tap still only scrolls; filters and accordions untouched |
| 2026-09-21 | The search page is rebuilt: left title, scope pills, one card per result, field at the bottom | **shipped** 2026-09-25 — `search-view.tsx` + `search-scope-pills.tsx` + `search-result-rows.tsx`, with the field in a sticky footer bar riding above the tab bar, and query and scope kept in the URL so Back restores the results. |
| 2026-09-21 | Search opens on recents; teams and games resolve live, players snapshot | **shipped** 2026-09-25 — `lib/recent-searches.ts` in `localStorage`, decided rather than inherited: per browser, surviving restarts, gone only with site data (the lifetime is argued in its header). |
| 2026-09-21 | Athletes in search, via ESPN's search endpoint, filtered to FBS and FCS | **shipped** 2026-09-25 — `/api/search/athletes`, filtered by club against the directory plus FCS, which it did not "load the same" (FBS only, Sun Belt empty — both fixed for the filter), and every league resolves its club because the web's player route needs a team id. |
| 2026-09-21 | The player page fetches its own profile and links its team badge | **shipped** — the page fetches `common/v3/.../athletes/{id}` from `site.web.api` (`web/src/lib/espn/athlete.ts`) beside the roster, the roster winning where both answer and standing alone when the athlete fetch fails; the badge links the payload's `team.id`, and `player-profile.ts`'s stale "no athlete endpoint" header is rewritten |
| 2026-09-21 | Search pushes on its own stack; Back returns to the results | **n/a** — a Next.js route push already behaves this way: the browser's Back is history, not a tab stack, so the web never had the bug this fixes. Recorded so the count matches and so nobody ports a fix for a problem that cannot occur there. |
| 2026-09-21 | Root tabs share one masthead, with a 52pt row floor | **shipped** — `web/src/components/page-header.tsx` replaces the four root tabs' screen-reader-only `<h1>`s with one visible left-aligned title row (Teams' plus in its trailing slot) on a 44px floor, the web's own tap-target number; all four measure the same header height and content offset. |
| 2026-09-21 | CFB becomes NCAAF in display; the spoken form stays "College Football" | **shipped** — both halves: `shortName` for `cfb` is "NCAAF" (2026-09-25), and every surface that shows it pairs it with the spoken "College Football" (`displayName`) — Scores section headers and Following cards ("SEC, College Football"), team cards, the Add teams sheet and search rows. A title that already names its league stays bare in both. |
| 2026-09-21 | Teams read by full name with "LEAGUE • Conference" beneath | **shipped** 2026-09-25 — `lib/team-name.ts`: the Teams tab card reads "Ohio State Buckeyes" over "NCAAF • Big Ten" (spoken "College Football Big Ten"), and the team hero wraps the full name to two lines; the hero's conference link and the search rows are unchanged here. |
| 2026-09-21 | Leagues accordions open closed, in memory for one run of the app | **differs** — deliberately. "Resets on force-quit" has no browser analog: a tab close is not a process end, and `sessionStorage` resets per tab rather than per app. The web should decide its own lifetime (most likely: collapsed on every load, nothing stored) rather than emulate a rule written for an app that can be quit. |
| 2026-09-21 | Find a league filters the hub and expands what it matched | **shipped** — `web/src/lib/hub-filter.ts` (tested) narrows both lists by iOS's rule, and a matched accordion opens through a render-time `isFiltering \|\| !isCollapsed` override that never writes to `use-ui-state`. |
| 2026-09-21 | The Leagues hub drops its standings teasers and puts its rows on one rail | **shipped** 2026-09-25 — the Following cards' teasers went with the Edit/Done port (#203); the league accordion rows lose the conference leader and Top 25's "#1" (and its spoken label); the league header drops its table count and moves to `type-team-name-em`; the heading below Following reads "All leagues, conferences, divisions"; Top 25 wears a trophy (`TrophyMark`, the same 24px box as `ConferenceLogo`) on both the accordion row and its Following card, while the Scores header keeps the league mark. The rail holds: header, Top 25 and conference rows all put the mark and the name on the same x. |
| 2026-09-21 | Games search reaches past the loaded window via the matched teams' schedules | **shipped** 2026-09-25 — `/api/team/[teamId]/schedule` + `lib/search-games.ts`; the cap stays three, reconsidered and kept for list length rather than request cost (argued at `SCHEDULE_SEARCH_MAX_TEAMS`). |
| 2026-09-21 | One player page, whichever door opened it | **shipped** — the badge now names the team off the fetched athlete payload (`displayName ?? location`) rather than the path's team, so a box-score link to a club the player has left opens his page badged with his current one; the stale-read half stays n/a |
| 2026-09-21 | The Leaders card's players link to their player pages | **shipped** — the web routed its Leaders card on `athleteId` first; this is iOS catching up, and `web/src/lib/player-profile.ts` can now drop the header caveat saying no athlete endpoint had been proved out. |
| 2026-09-21 | The player hero is a name and a club; number and position are Profile rows | **shipped** — `playerMetaLine` is retired and the hero is the name over the team badge; `playerSpokenSummary` is name and club only, as iOS's `spokenSummary` |
| 2026-09-21 | A team in the Add teams sheet reads as one name | **shipped** 2026-09-25 — `team-follow-row.tsx` prints one `teamFullName` string, weighted by follow state, clamped to two lines rather than truncated; the league tag moves to the trailing edge. |
| 2026-09-22 | Hide all/Show all control on Scores, scoped to sections you don't follow | **shipped** — `GameSection.isFollowed` on Following and hoisted tables, `splitAtHideAll` in `game-sections.ts`, a `HideAllControl` between the two groups, `hideOtherSections` persisted in `statside.ui.v1`; the hidden stack closes on the accordion curve |
| 2026-09-24 | The app opts into ProMotion (`CADisableMinimumFrameDurationOnPhone`) | **n/a** — an iOS Info.plist key. Browsers already drive `requestAnimationFrame` and CSS transitions at the display's refresh rate. |
| 2026-09-24 | The Scores screen stops rebuilding itself per frame: sections memo, cached day slots, equatable strip and accordions | **n/a** — a SwiftUI invalidation problem. The web's `game-sections.ts` runs once per render and React already skips unchanged props; revisit only if a profile says otherwise. |
| 2026-09-24 | Section open/close is a 0.18s ease-out, not a spring | **shipped** — it was a 0.25s material curve; `ACCORDION_TRANSITION` in `lib/motion.ts` (0.18s, `[0, 0, 0.58, 1]`) now drives the accordion and Hide all alike, instant under reduced motion |
| 2026-09-24 | Render path stops redoing fixed work: cached fonts, logos decoded off main, athlete decode off main | **n/a** — browsers decode images off the main thread already, and the web has no Dynamic Type font bridge. |
| 2026-09-24 | The live dot pulses in Core Animation on one shared clock | **differs** — the web's dot is a CSS animation, which is already compositor-driven; the half worth porting is the shared phase (dots in sync), via a common `animation-delay` from page load. |
| 2026-09-24 | Player stats from `site.web.api`; the player page's Profile · Games · Stats · Career tabs and This season card | **shipped** — stats server-side on page load (no stats line, no tab row), the game log on the Games tab's first open via `/api/player/[athleteId]/gamelog` with a season chip over ESPN's own season list, and a This season card on Profile keyed to the app-wide `seasonYearContaining` clock |
| 2026-09-24 | Career is ESPN's rows and totals; headline numbers keyed by category name | **shipped** — `headlineNames` in `web/src/lib/player-stats.ts` is a straight copy with the registry pinned by tests; Career prints ESPN's rows and `totals` unsummed, captioning each season with the stats payload's own club abbreviation where iOS asks its team directory |
| 2026-09-24 | Box score names link to player pages, real athlete ids only | **shipped** — the web linked box-score rows first (`BoxScorePlayer.athleteId`); iOS now matches. |
| 2026-09-24 | Team stats: Season stats and Leaders cards on Overview, a full Stats tab with football's Team / Opponents switch | **shipped** 2026-09-25 — `lib/espn/team-stats.ts` copies both registries; the page streams stats (`site.web.api`) and core-API leaders into Overview cards and a Stats tab after Games, the switch drawn only when the payload carries opponents; leaders link where the current roster can open them. |
| 2026-09-24 | ESPN's rollover season label corrected by games played; leaders fall back a season and say so | **shipped** 2026-09-25 — `teamStatsSeasonLabel` (20-game floor) and `teamLeaders`' one-season fallback on the `seasonYearContaining` clock, both unit-tested. |
...
| 2026-09-24 | Tight filter (live, late within one score, or underdog leading) beside Live | **shipped** — `game-closeness.ts` is a straight copy of the rule and its per-league table; the scoreboard transform reads `odds[0]`'s `favorite` flags into `Game.favoriteIsHome`, carried past a payload that drops it; a Tight pill in ink beside Live, exclusive with it and suspended off today the same way |
...
| 2026-09-24 | Win probability card: predictor split bar pre-game, per-play line after | **shipped** 2026-09-25 — the data half (`transformWinProbability`: series once it has two points, else the predictor); the bar and line were superseded the same day by the row below, so no chart was built. |
| 2026-09-24 | Win probability card as one FotMob-style row; caption names the moment (predictor / live / at kickoff) | **shipped** 2026-09-25 — `win-probability-card.tsx` after the line score, favorite in `type-score` ink, caption in the header's trailing slot, one `sr-only` sentence. |
| 2026-09-24 | Leagues list A–Z (NBA, NCAAF, NFL, NHL) on Scores and the tables hub | **shipped** — `LEAGUE_DISPLAY_ORDER` (sorted by short name) drives the Scores stack, the Leagues hub's accordions and a new follow's default position; `LEAGUES` keeps declaration order for fetches and spans |
...
| 2026-09-25 | Hide all capsule hugs its contents, centered, not full width | **shipped** — built as a centered inline pill (`px-4`, chip type, chevron) from the start |
| 2026-09-25 | Settings: Appearance picker (System / Light / Dark), System default | **shipped** — `AppearancePicker` in the site footer (the web has no Settings page) drives the existing `next-themes` `.dark` class, stored as `statside.appearance` and applied by its pre-paint script, so no flash. |
...
| 2026-09-25 | Scoreboard polls through kickoff (30s from one tick before a kickoff to 3h after it, sleeping until the next one); Top 25 and Postseason merge the live board | **shipped** — `poll-schedule.ts` ports `nextPollDelay`; `use-league-scoreboards.ts` replaces its live-only interval with a timeout chain that sleeps to the next kickoff (capped at an hour, no request on wake) and asks nothing while the tab is hidden. The Top 25/Postseason merge half has no web shape: there is no app-wide board to merge from, and those pages render their own snapshot |
| 2026-09-25 | Leagues Following: Edit/Done link on the heading; dismiss + grip only in edit mode; no star or hold-to-lift outside it; drop resolved against visible neighbours | **shipped** — web port PR: Edit/Done on the Following heading; minus + grip only in edit mode, grip lifts on press; Move up/down kept for keyboard and screen readers, visually hidden until focused; `moveAmongVisible` saves the whole order, so a hidden table no longer drops out of it. |
...
| 2026-09-25 | The player page always shows its four tabs; a player with no stats line gets empty states rather than Profile alone | **shipped** — both platforms in one PR. Web: `player-view.tsx` has one code path, with "No stats this season" / "No career stats yet" in the Stats and Career panes. iOS: `PlayerPage` drops `availableTabs` and gains the same two empty states. Both skip categories with no season lines. |
| 2026-09-26 | Scoreboard and game-summary polls skip the local HTTP cache | **n/a** — the web fetches ESPN server-side through Next's data cache on its own 30s `revalidate`; there's no client HTTP cache in that path to go stale. |
| 2026-09-26 | The Scores list takes the game page's number when it's further along, and never moves a game backwards | **pending** — the web's scoreboard and game page are separate server fetches with nothing shared between them; the no-backwards rule would need a client-side hold on the scores page. |
| 2026-09-26 | Live polls run every 1s, not 30s | **shipped** — `POLL_INTERVAL_LIVE` is 1s (drives both `useLeagueScoreboards` and `useLiveGame`), and the provider's `REVALIDATE` for the scoreboard and game summary drops to 1s, which still coalesces every visitor's tick into one ESPN request a second. Next serves one stale copy while it revalidates, so a web row can trail ESPN by one tick. |
| 2026-09-26 | Live polls ask only for the days with games in play; the whole window refreshes every 30s | **shipped**: `use-league-scoreboards.ts` asks `/api/scoreboard?dates=` for the live days between 30s window refreshes (`@/lib/live-days`). The browser computes the Eastern tokens itself, because the server's day helpers read the host clock (UTC on Vercel) and a one-day ask has no margin to absorb that. |
| 2026-09-26 | Settings switches use the app's green as their dark-mode "on" track | **n/a** — the web has no Settings switches: betting lines aren't a web setting (2026-09-24 row, `differs`) and its Appearance control is a picker. |
| 2026-09-26 | Following with no games that day: a "Following / No games today" header stands in, with Hide all/Show all under it | **pending** — `splitAtHideAll` in `game-sections.ts` still needs a followed section on the day; the web wants the same stand-in and the follows-anyone check |
| 2026-09-26 | Hide all/Show all: the top three sections close to headers and gather into a deck, the rest fade in place, the deck fades up and uncovers the caption beneath it, all in about 0.25s with a strong ease-out (reverse to show; interruptible; a plain crossfade under reduced motion); caption 12pt under the capsule | **pending** — the web's hidden stack closes on the 0.18s accordion curve; it needs the three-step motion, the caption revealed from under the deck, `prefers-reduced-motion` and the wider caption gap |
