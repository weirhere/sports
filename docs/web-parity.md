...
| 2026-09-25 | The web player page always shows its four tabs; a player with no stats line gets empty states rather than Profile alone | **differs** — Andy asked for one tabbed page on the web: opening a freshman from a roster gave a tabless page while the next row gave a tabbed one. The web now always draws Profile · Games · Stats · Career, with "No games this season", "No stats this season" and "No career stats yet" where ESPN has nothing. iOS still hides the tabs without a stats line (`PlayerPage.swift` `availableTabs`). |
