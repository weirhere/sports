...
| 2026-09-21 | The player page fetches its own profile and links its team badge | **pending** — and it **supersedes a comment in the web source**. `web/src/lib/player-profile.ts` says no athlete endpoint had been proved out; one has, and that header should be corrected whenever this is ported, not left to imply the question is still open. |
...
| 2026-09-21 | One player page, whichever door opened it | **partly n/a** — the web has one door (a roster link under `/player/[league]/[teamId]/[athleteId]`), so it cannot have had the disagreement. The rule still ports as a rule: name the team off the fetched team, not off whatever the linking page happened to hold. The stale-read half has no analog — the web page is server-rendered from one source and has no "door's copy" to go stale. |
...
| 2026-09-21 | The player hero is a name and a club; number and position are Profile rows | **pending** — `player-view.tsx` draws exactly the line being removed (`playerMetaLine(player)` beside the team badge). Port both halves: the visible line and the `playerSpokenSummary` in the `sr-only` span above it, which carries the same number and position. |
...
| 2026-09-24 | Player stats from `site.web.api`; the player page's Profile · Games · Stats · Career tabs and This season card | **pending** — `web/src/lib/player-profile.ts`'s "no endpoint we've proved out" header is now false. The web can call the same `common/v3` athlete `/stats` and `/gamelog` endpoints server-side, which also sidesteps the browser-UA 403s `site.api` hands out. |
| 2026-09-24 | Career is ESPN's rows and totals; headline numbers keyed by category name | **pending** — port with the tabs above; the headline registry should be a straight copy of `PlayerStats.headlineNames(for:)`. |
