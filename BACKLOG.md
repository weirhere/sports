...
- [ ] **P1** Rankings tab becomes the tables hub: the poll renders exactly as today (picker, zero taps to the Top 25), then a CONFERENCES section below — one row per known conference (logo, name, leader teaser like "Ole Miss · 7-1 conf") pushing its ConferencePage, with a trailing follow star (same `FollowingStore.toggleConference` bit as the page pill). Followed conferences pin above the rest; then tier → name (the standings fetch's order). Teaser hidden preseason (a 0-0 "leader" is last season's carry-over, not information). Acceptance: poll experience unchanged; conference list renders offseason (rows without teasers); a follow star pins the row on next appearance and the Scores Following section reflects it; conferences section hides entirely if the standings fetch fails (never an error over the poll); rows and stars accessible (row speaks name + teaser, star speaks follow state). Decided 2026-08-10 after rejecting a 4th tab — a Conferences tab would be a menu, not a destination; Rankings already answers "who's good" and now answers it at both levels.
