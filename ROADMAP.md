...
- ~~**Any App Store release.**~~ **Shipped** — 1.0 went live 2026-08-23 on ESPN, which crossed the data-licensing gate by taking the risk rather than resolving it. **Monetization is still later, and now scoped** *(2026-09-14 as BACKLOG E18, with `docs/monetization.md` carrying the reasoning)*: it forces the same question with money changing hands, which is a different risk posture than a free personal app. Memberships fit the constraints (StoreKit 2 is first-party, entitlement is per Apple ID, no accounts and no backend needed); ads do not (every network is a data-collecting SDK, and the README sells an app for people tired of ad-stuffed everything-apps). The epic's first item is the go/no-go, and it is a quote, not a task.
...
| Seasonality (Feb–Aug is dead air) | Accepted for v1. Offseason landing state in Phase 4. **CBB is no longer the answer** (2026-09-14): it runs Nov–Mar and overlaps what the app already covers. MLB (Feb–Nov) and the WNBA (Apr–Oct) are the leagues that cover the hole — scoped as BACKLOG E17, not scheduled |
