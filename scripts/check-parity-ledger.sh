...
#
# Escape hatch: put `[skip-parity]` in the PR title or any commit message on
# the branch. Use it for iOS work with genuinely no product surface (a test
...
# Rows in the ledger are table lines too — either a dated iOS row or an
# `n/a` row, both of which count as "you wrote something down".
ledger_rows() {
  git show "$1:$LEDGER_FILE" 2>/dev/null | grep -cE '^\| [0-9]{4}-[0-9]{2}-[0-9]{2} \||^\| — \|' || true
}

if git log "$BASE_REF..HEAD" --format='%B' 2>/dev/null | grep -qF '[skip-parity]'; then
  echo "✓ [skip-parity] found in a commit message — ledger check skipped."
  exit 0
fi
...
✗ This branch adds $added_decisions decision row(s) to $CLAUDE_FILE but only
  ...
  If this branch genuinely has no product surface, put [skip-parity] in a
  commit message.
EOF
exit 1
