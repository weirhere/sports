...
| 2026-08-09 | Scores and Teams stacks switched typed paths (`[Game]`/`[Team]`) to `NavigationPath` | A homogeneous typed path can't push a second destination type; `NavigationPath` keeps every existing `NavigationLink(value:)` and the deep-link pending-id resolution intact. Rankings keeps its view-builder links but registers the same value destinations |
| 2026-08-10 | Rankings is the tables hub: CONFERENCES card (one row per conference, leader teaser, follow star, followed pinned first) below the unchanged poll — no 4th tab | Andy's call after weighing a Conferences tab: a tab would be a menu, not a destination, and Rankings already answers "who's good" — now at both levels. The poll stays zero taps away; the teaser hides preseason (a 0-0 "leader" is last season's carry-over); a standings fetch miss hides the card, never errors over the poll |
