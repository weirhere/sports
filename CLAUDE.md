...
| 2026-08-24 | TeamPage's standing line becomes the header's conference affordance (`TeamConferenceLink`): conference mark + standing in season, the bare conference name when the standing hides (offseason 0-0, past seasons, CFBD) — one tap target → ConferencePage. The id resolves schedule-payload-first (`team.groups`, now decoded; season-scoped, so it's deliberately outside the summaries trust rule and realignment reads correctly per selected season) → pushed team → `TeamDirectoryStore` lookup | The only conference link on a team page was the standing string, which vanished offseason, on past seasons, and for teams pushed from Rankings/game detail (nil `conferenceId`). Merging into one element avoids double-saying "SEC" in season; CFBD parity came free via `Conference.id(forCFBDName:)` |
